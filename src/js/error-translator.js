...
const ERROR_PATTERNS = [
  // Syntax errors
  {
    ...
    suggestion: 'Check for missing semicolons, brackets, or parentheses in your OpenSCAD file.'
  },
  {
    pattern: /unexpected token/i,
    title: 'Unexpected Character',
    explanation: 'The model code has a character in an unexpected place.',
    suggestion: 'Review the code near the line number shown for extra or missing punctuation.'
  },
  {
    pattern: /undefined variable[:\s]+(\w+)/i,
    title: 'Missing Variable',
    explanation: (match) => `The model references a variable called "${match[1]}" that doesn't exist.`,
    suggestion: 'Make sure all parameter names are spelled correctly and defined before use.'
  },
  {
    pattern: /unknown function[:\s]+(\w+)/i,
    title: 'Unknown Function',
    explanation: (match) => `The model uses a function called "${match[1]}" that OpenSCAD doesn't recognize.`,
    suggestion: 'This might be from a library that\'s not enabled. Check the Libraries panel.'
  },
  ...
  // Geometry errors
  {
    ...
    title: 'Shape Problem',
    explanation: 'The model created a shape with invalid geometry (like overlapping edges).',
    suggestion: 'Try adjusting dimensions to create simpler shapes, or reduce the number of sides.'
  },
  {
    pattern: /non-planar face/i,
    title: 'Geometry Issue',
    explanation: 'One of the surfaces in the model isn\'t flat when it should be.',
    suggestion: 'This can happen with complex extrusions. Try simplifying the shape.'
  },
  {
    pattern: /degenerate/i,
    title: 'Invalid Shape',
    explanation: 'The current parameter values created a shape that\'s too thin or has no volume.',
    suggestion: 'Increase dimension values or change parameters to create a valid 3D shape.'
  },
  ...
  // Memory and timeout errors
  {
    ...
    suggestion: 'Try reducing complexity: lower $fn values, simpler shapes, or fewer operations.'
  },
  {
    pattern: /timeout|timed out|too long/i,
    title: 'Taking Too Long',
    explanation: 'The model is taking too long to generate.',
    suggestion: 'Reduce $fn values, simplify the model, or try the "Fast" preview quality setting.'
  },
  ...
  // File errors
  {
    ...
    title: 'Missing File',
    explanation: (match) => `The model needs a file called "${match[1]}" that wasn't found.`,
    suggestion: 'If using a ZIP file, ensure all referenced files are included.'
  },
  {
    pattern: /cannot open/i,
    title: 'File Access Problem',
    explanation: 'OpenSCAD couldn\'t read one of the required files.',
    suggestion: 'Try re-uploading the file or check if all included files are present.'
  },
  ...
  },
  {
    ...
    explanation: (match) => `The model includes "${match[1]}" which wasn't found.`,
    suggestion: 'This file might be from an external library. Check the Libraries panel.'
  },
  ...
  // CGAL errors (common in complex boolean operations)
  {
    ...
    explanation: 'The model\'s shapes have geometry that\'s difficult to process.',
    suggestion: 'Try simpler parameter values, or avoid very thin walls or sharp angles.'
  },
  ...
  // Render quality errors
  {
    ...
    explanation: (match) => `The $fn value of ${match[1]} might be causing problems.`,
    suggestion: 'Try a lower $fn value (32-64 is usually sufficient for most models).'
  },
  ...
  // Generic parameter errors
  {
    ...
    suggestion: 'Check your parameter values and try using values closer to the defaults.'
  }
];
...
const DEFAULT_ERROR = {
  ...
  suggestion: 'Try resetting parameters to defaults, or try a simpler model first.'
};
...
export function translateError(technicalError) {
  ...
  for (const pattern of ERROR_PATTERNS) {
    ...
    if (match) {
      return {
        title: pattern.title,
        explanation: typeof pattern.explanation === 'function' 
