...
${scadContent}
  ...
  <body>
    <div id="app">
      ...
      <footer>
        <p>Generated by OpenSCAD Assistive Forge • <a href="https://github.com/BrennenJohnston/openscad-assistive-forge">Source Code</a></p>
      </footer>
    </div>
...
[OpenSCAD Assistive Forge](https://github.com/BrennenJohnston/openscad-assistive-forge) - Developer toolchain for OpenSCAD Customizer projects
`;
}
...
export async function scaffoldCommand(options) {
  try {
    ...
    try {
      ...
      console.error(chalk.red(`✗ Failed to read schema: ${err.message}`));
      process.exit(1);
    }
    ...
    try {
      ...
      console.error(chalk.red(`✗ Failed to read OpenSCAD file: ${err.message}`));
      process.exit(1);
    }
    ...
    if (options.template === 'vanilla') {
      ...
      if (!existsSync(srcDir)) {
        console.error(chalk.red(`✗ ${options.template.charAt(0).toUpperCase() + options.template.slice(1)} template directory not found: ${srcDir}`));
        process.exit(1);
      }
    } else {
    ...
    try {
      ...
      console.error(chalk.red(`✗ Failed to copy source files: ${err.message}`));
      process.exit(1);
    }
    ...
    if (existsSync(publicDir)) {
      try {
        ...
        console.log(chalk.green('✓ Copied public assets'));
      } catch (err) {
        console.error(chalk.red(`✗ Failed to copy public assets: ${err.message}`));
        process.exit(1);
      }
    }
