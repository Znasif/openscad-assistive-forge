 ...
 * @type {Object.<string, {name: string, description: string, colors: Object}>}
 */
 ...
 */
export function generateThemeCSS(colors, themeName = 'Custom') {
  return `/**
 * ${themeName} Theme
...
export async function themeCommand(options) {
  try {
    console.log(chalk.blue('🎨 OpenSCAD Forge - Theme Generator'));
    ...
    if (existsSync(resolvedPath) && !options.force) {
      console.error(chalk.red(`✗ File already exists: ${resolvedPath}`));
    ...
    if (options.custom) {
      ...
      if (!options.primary && !options.secondary && !options.background) {
        console.error(chalk.red('✗ Custom theme requires at least one color'));
      ...
      if (options.primary && !isValidHexColor(options.primary)) {
        console.error(chalk.red(`✗ Invalid primary color: ${options.primary}`));
        process.exit(1);
      }
      if (options.secondary && !isValidHexColor(options.secondary)) {
        console.error(chalk.red(`✗ Invalid secondary color: ${options.secondary}`));
        process.exit(1);
      }
      if (options.background && !isValidHexColor(options.background)) {
        console.error(chalk.red(`✗ Invalid background color: ${options.background}`));
        process.exit(1);
      }
      ...
      themeName = 'Custom';
      console.log(chalk.green('✓ Generated custom theme'));
      ...
      console.log(chalk.green(`✓ Using preset: ${themeName}`));
    } else {
      // Default to blue theme
      colors = THEME_PRESETS.blue.colors;
      themeName = THEME_PRESETS.blue.name;
      console.log(chalk.green(`✓ Using default theme: ${themeName}`));
    }
    ...
    console.log(chalk.green(`✓ Theme saved to: ${resolvedPath}`));
